...
class LuminaAI {
  ...
  static const String _kSmartSystemPrompt =
      ...
      'When the user wants a team\'s lighting on EVERY game, every home game, '
      'all season, or across a date range of games (NOT a single game), emit '
      'a `recurringSportsAutopilot` rule. This is a tiny rule the app expands '
      'into the real schedule — DO NOT list individual game dates.\n'
      '`recurringSportsAutopilot` schema:\n'
      '  {\n'
      '    "teamSlug": "<sport>_<team>",\n'
      '    "untilDate": "YYYY-MM-DD" | null\n'
      '  }\n'
      'CRITICAL — NO ENUMERATION: never produce one entry per game. The whole '
      'point of this rule is that the app pulls the team\'s real schedule and '
      'lights up each game automatically. A request covering a whole season is '
      'this ONE small object, never 80 dated entries.\n'
      'When to use:\n'
      '• "Royals colors every home game", "Chiefs every game this season", '
      '"light up for all Lakers games", "Royals blue every home-game night '
      'through October". → recurringSportsAutopilot populated.\n'
      '• A SINGLE game (with or without an after-state) → use ephemeralSession '
      '(or just wled), NOT this rule.\n'
      '• Clock-time recurrence with no team ("warm white every Friday") → '
      'schedulingIntent, NOT this rule.\n'
      'teamSlug: same format and catalog as ephemeralSession.teamSlug — '
      '`<sport>_<team>` snake_case (e.g. mlb_royals, nfl_chiefs, nba_lakers). '
      'The app resolves the team name, colors, ESPN id, and sport from the '
      'slug, so you do NOT emit those.\n'
      'untilDate: set ONLY when the user gives an explicit end ("through '
      'October" → "2026-10-31", "until the playoffs start on Sep 30" → '
      '"2026-09-30"). Otherwise null — the season\'s natural end governs.\n'
      'EXCLUSIVITY: when you emit recurringSportsAutopilot, set '
      'schedulingIntent/schedulingIntents AND ephemeralSession to null, and do '
      'NOT also produce enumerated changes. `wled` is OPTIONAL here (the app '
      'uses the team\'s colors) — you may include a representative team design '
      'for the preview, but keep the response compact.\n'
      'EXAMPLE — user: "Royals blue for every home game night through October '
      '2026":\n'
      '   → recurringSportsAutopilot: {"teamSlug":"mlb_royals",'
      '"untilDate":"2026-10-31"}, schedulingIntent: null, '
      'ephemeralSession: null\n\n'
