    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // Item #72 migration debug entrypoint — kDebugMode + email
          // gate so it's invisible in release builds AND in debug
          // builds signed in as anyone other than the founder
          // account. Delete this block after the migration completes
          // and the cleanup commit lands.
          const _Item72MigrationDebugCard(),
...
/// Item #72 migration debug invocation card.
///
/// Visible ONLY when:
///   - Running a debug build (kDebugMode == true), AND
///   - The signed-in Firebase Auth user's email matches the
///     founder-admin allowlist (currently just
///     honeycutt.tylerg@gmail.com).
///
/// Renders nothing otherwise — release builds and other accounts
/// (including admin accounts on other Lumina installs) see no
/// surface. Calls the deployed v2 onCall
/// `migrateClearScheduleItemsV1` via the user's real Firebase Auth
/// session, which carries the email needed to satisfy the Cloud
/// Function's auth gate.
///
/// Idempotent on the server side (config/migrations/items/* tracking
/// doc) so accidental double-taps are safe. UI confirmation dialog
/// added on top of that for explicit consent.
///
/// DELETE THIS WIDGET (plus the import + the ListView entry) after
/// the migration completes and the cleanup commit lands.
class _Item72MigrationDebugCard extends StatefulWidget {
  const _Item72MigrationDebugCard();

  @override
  State<_Item72MigrationDebugCard> createState() =>
      _Item72MigrationDebugCardState();
}

class _Item72MigrationDebugCardState
    extends State<_Item72MigrationDebugCard> {
  // Mirrors the Cloud Function's auth gate
  // (functions/src/migrateClearScheduleItemsV1.ts): allowlist set
  // for the founder Gmail PLUS @nex-genled.com domain. So whichever
  // identity is signed in (Gmail or work email), the button is
  // visible only when the function would actually accept the call.
  static const _kAllowedEmail = 'honeycutt.tylerg@gmail.com';
  static const _kAllowedDomain = '@nex-genled.com';
  ...
  bool get _shouldShow {
    ...
    if (email == _kAllowedEmail) return true;
    if (email.endsWith(_kAllowedDomain)) return true;
    return false;
  }
  ...
  Future<void> _confirmAndRun() async {
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Run Item #72 Migration?'),
        content: const Text(
          'Clears the `schedules` array on EVERY user document in '
          'Firestore. Idempotent (won\'t double-run). Returns the '
          'usersAffected + totalSchedulesCleared counts.\n\n'
          'Continue?',
        ),
        actions: [
          TextButton(
    ...
    try {
      final callable = FirebaseFunctions.instanceFor(region: 'us-central1')
          .httpsCallable('migrateClearScheduleItemsV1');
      final res = await callable.call<Map<String, dynamic>>(<String, dynamic>{});
      resultText = res.data.toString();
      // Also dump to debugPrint so it lands in the active log stream.
      debugPrint('Item72Migration: response = $resultText');
    } on FirebaseFunctionsException catch (e) {
      resultText = 'FirebaseFunctionsException: code=${e.code}, '
          'message=${e.message}, details=${e.details}';
      debugPrint('Item72Migration: FAILED — $resultText');
    } catch (e) {
      resultText = 'Unexpected error: $e';
      debugPrint('Item72Migration: FAILED — $resultText');
    }
    if (!mounted) return;
    ...
    await showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Migration Response'),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Clear ScheduleItems across the entire user '
                'collection. Idempotent. Server-side response is shown '
                'in a dialog and logged via debugPrint.',
