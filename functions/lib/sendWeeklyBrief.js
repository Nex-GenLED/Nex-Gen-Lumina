...
exports.sendWeeklyBrief = void 0;
...
// Constants
// ---------------------------------------------------------------------------
const SYSTEM_PROMPT = "You are Lumina, the AI behind a smart LED lighting app. " +
 ...
 */
function getUpcomingWeekWindow() {
    const now = new Date();
 ...
 */
function callClaudeHaiku(apiKey, systemPrompt, userMessage) {
...
// Cloud Function
// ---------------------------------------------------------------------------
exports.sendWeeklyBrief = (0, scheduler_1.onSchedule)({
    ...
    console.log(`sendWeeklyBrief: found ${usersSnapshot.size} eligible users`);
    let sentCount = 0;
    let skipCount = 0;
    let errorCount = 0;
    for (const userDoc of usersSnapshot.docs) {
        ...
        try {
            ...
            if (events.length === 0) {
                ...
                const userMessage = `User's first name: ${firstName}\n` +
                    `Upcoming week events (${events.length} total):\n` +
                    eventLines.join("\n");
                try {
                    notificationBody = await callClaudeHaiku(apiKey, SYSTEM_PROMPT, userMessage);
                    notificationBody = notificationBody.slice(0, 120);
                    // Sanity check
                    ...
                    console.warn(`sendWeeklyBrief: AI failed for ${uid}, using fallback:`, aiErr);
                    notificationBody =
                        `${firstName}, your lights are set to warm white all week. Tap to customize.`;
                    notificationBody = notificationBody.slice(0, 120);
                }
    ...
    console.log(`sendWeeklyBrief: done — sent=${sentCount}, skipped=${skipCount}, errors=${errorCount}`);
});
//# sourceMappingURL=sendWeeklyBrief.js.map