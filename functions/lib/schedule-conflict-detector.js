...
function detectConflicts(proposed, existing) {
    ...
    for (const entry of proposed) {
        ...
        for (const event of existing) {
            ...
            const zoneOverlap = entry.zone === "all" ||
                event.zone === "all" ||
            ...
            if (timeRangesOverlap(entryStart, entryEnd, eventStart, eventEnd)) {
                ...
                const daysLabel = sharedDays.length <= 3
                    ? sharedDays.join(", ")
 ...
 */
function suggestResolution(proposed, existing) {
...
//# sourceMappingURL=schedule-conflict-detector.js.map