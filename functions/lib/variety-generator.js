...
exports.generateVarietyPlan = generateVarietyPlan;
exports.validateVarietyPlan = validateVarietyPlan;
 ...
 */
function rotateColors(themeColors, dayIndex) {
 ...
 */
function pickEffect(dayLabel, dayIndex, previousEffectId, preferredEffects, festive) {
 ...
 */
function pickParams(dayIndex, energy) {
 ...
 */
function generateVarietyPlan(days, config) {
    const entries = [];
    ...
    for (let i = 0; i < days.length; i++) {
        ...
        const effectId = pickEffect(dayLabel, i, previousEffectId, config.preferredEffects, config.festive);
        // Rotate colors
 ...
 */
function validateVarietyPlan(entries) {
    for (let i = 1; i < entries.length; i++) {
        const prev = entries[i - 1];
        const curr = entries[i];
        // Check if effect AND primary color are identical
        if (prev.effectId === curr.effectId &&
...
//# sourceMappingURL=variety-generator.js.map