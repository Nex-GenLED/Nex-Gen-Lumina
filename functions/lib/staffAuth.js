 ...
 */
function masterConfigForMode(mode, pin) {
    switch (mode) {
        case "sales":
...
async function checkRateLimit(db, ip) {
    try {
        ...
        firebase_functions_1.logger.error(`mintStaffToken: rate limit check failed (failing open): ${err.message}`);
        return false;
    }
}
/**
 ...
 */
async function tryMasterPin(db, mode, pin, hashHex) {
 ...
 */
async function tryInstallersDoc(db, pin, mode) {
    ...
    if (docRole !== roleForMode(mode))
        return null;
    const dealerCode = data.dealerCode ?? pin.substring(0, 2);
 ...
 */
async function isDealerActive(db, dealerCode) {
    const snap = await db
        .collection("dealers")
...
async function writeAuditLog(db, entry) {
    try {
        ...
        firebase_functions_1.logger.error(`mintStaffToken: audit log write failed: ${err.message}`);
    }
}
// ── Cloud Function ──────────────────────────────────────────────────────────
exports.mintStaffToken = (0, https_1.onCall)({
    region: "us-central1",
    maxInstances: 10,
}, async (request) => {
    ...
    if (!PIN_REGEX.test(pin)) {
        throw new https_1.HttpsError("invalid-argument", "pin must be 4-6 digits");
    }
    // ── 2. Validate mode ────────────────────────────────────────────────
    if (mode !== "sales" &&
        mode !== "installer" &&
        mode !== "admin" &&
        mode !== "owner") {
        throw new https_1.HttpsError("invalid-argument", "mode must be 'sales', 'installer', 'admin', or 'owner'");
    }
    ...
    if (await checkRateLimit(db, rateLimitKey)) {
        ...
        throw new https_1.HttpsError("resource-exhausted", "Too many attempts. Try again in a minute.");
    }
    // ── 4. Hash PIN ─────────────────────────────────────────────────────
    ...
    // obvious.
    let resolved = await tryMasterPin(db, mode, pin, hashHex);
    if (!resolved) {
    ...
    if (!resolved) {
        ...
        throw new https_1.HttpsError("permission-denied", "Authentication failed");
    }
    ...
    try {
        ...
        firebase_functions_1.logger.error(`mintStaffToken: createCustomToken failed for ${uid}: ${errCode}`);
        await writeAuditLog(db, {
            mode,
            success: false,
            source: "mint_failed",
        ...
        throw new https_1.HttpsError("internal", "Failed to mint staff token");
    }
