...
exports.buildSchedulingSystemPrompt = buildSchedulingSystemPrompt;
// ---------------------------------------------------------------------------
// Effect Reference
// ---------------------------------------------------------------------------
 ...
 */
function buildSchedulingSystemPrompt(context) {
    // Format current schedule
    const currentScheduleStr = context.currentSchedule.length > 0
        ? context.currentSchedule
            ...
            const colorsStr = e.colors
                .map((c) => `[${c.join(",")}]`)
                .join(", ");
            const trigger = e.triggerType === "clock"
                ? e.startTime
        ...
        : "  (no events scheduled)";
    // Format user teams
    const teamsStr = context.userTeams.length > 0
        ? context.userTeams
            .map((t, i) => `  ${i + 1}. ${t.name} (${t.league}) — primary: [${t.primaryColor.join(",")}], secondary: [${t.secondaryColor.join(",")}]${t.accentColor ? `, accent: [${t.accentColor.join(",")}]` : ""}`)
            .join("\n")
        : "  (no teams configured)";
...
//# sourceMappingURL=scheduling-system-prompt.js.map