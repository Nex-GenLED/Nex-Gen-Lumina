...
export const SYNC_FANOUT_SOURCE = "sync_fanout";
 ...
 */
export const SYNC_COMMAND_TTL_MS = 90_000;
 ...
 */
export const BRIDGE_LIVE_WINDOW_MS = 120_000;
 ...
 */
export const STUCK_EXECUTING_MS = 90_000;
 ...
 */
export function classifyDeliveryRoute(args: {
  webhookUrl: string | null;
  bridgeHeartbeatMs: number | null;
  nowMs: number;
}): { route: DeliveryRoute; reason: string } {
  const hasWebhook =
    typeof args.webhookUrl === "string" && args.webhookUrl.trim().length > 0;
  if (hasWebhook) return { route: "webhook", reason: "webhook_url" };
  ...
  if (ageMs > BRIDGE_LIVE_WINDOW_MS) {
    return {
      route: "no_bridge",
 ...
 */
export function syncCommandDocId(
  fireId: string,
  controllerId: string,
  ordinal: number
): string {
 ...
 */
export function planQueueHygiene(
  docs: InFlightDoc[],
  nowMs: number
): { supersede: string[]; stuck: string[] } {
  ...
  for (const d of docs) {
    if (d.status === "pending") {
      ...
      if (d.createdAtMs !== null && nowMs - d.createdAtMs > STUCK_EXECUTING_MS) {
        stuck.push(d.id);
      }
    }
  }
...
function memberDisplayName(data: admin.firestore.DocumentData): string {
 ...
 */
async function planMemberDelivery(
  db: admin.firestore.Firestore,
  args: {
    memberUid: string;
    memberData: admin.firestore.DocumentData;
  ...
  const bare = (status: string, reason: string): MemberPlan => ({
    targets: [
      {
        uid: args.memberUid,
        displayName,
  ...
  try {
    ...
    let bridgeHeartbeatMs: number | null = null;
    try {
      const hb = await userRef.collection("bridge_status").doc("current").get();
      if (hb.exists) {
        const t = (hb as { updateTime?: { toMillis(): number } }).updateTime;
    ...
    const records: FireTargetRecord[] = [];
    ...
    if (route.route === "no_bridge") {
      ...
      console.warn(
        `applySyncPattern FANOUT: ${args.memberUid} NOT commanded — ` +
    ...
    const inFlight: InFlightDoc[] = [];
    for (const status of ["pending", "executing"]) {
      try {
        const snap = await commandsRef
          .where("source", "==", SYNC_FANOUT_SOURCE)
          .where("status", "==", status)
          .get();
        snap.forEach((d) => {
          const dd = d.data() || {};
          const created = dd.createdAt as { toMillis?: () => number } | undefined;
          inFlight.push({
            id: d.id,
            status,
            createdAtMs:
        ...
        console.warn(
          `applySyncPattern FANOUT: hygiene read (${status}) failed for ${args.memberUid}`,
          err
        );
      }
    }
    ...
    for (const id of hygiene.stuck) {
      writes.push({
        ...
        data: {
          status: "failed",
          error:
            "stuck_executing: bridge claimed this command and never reported " +
            `(> ${STUCK_EXECUTING_MS / 1000}s); cleared by sync fire ${args.fireId}.`,
          completedAt: admin.firestore.FieldValue.serverTimestamp(),
        },
      });
    }
    if (hygiene.supersede.length + hygiene.stuck.length > 0) {
      console.warn(
        `applySyncPattern FANOUT: hygiene for ${args.memberUid} — ` +
    ...
    targets.forEach((t, i) => {
      ...
      if (!part.partitioned) {
        console.log(
          `applySyncPattern FANOUT: ${args.memberUid}/${t.id} NOT partitioned ` +
...
export async function fanoutToCrew(
  ...
  const expiresAtMs = nowMs + SYNC_COMMAND_TTL_MS;
  const expiresAt = admin.firestore.Timestamp.fromMillis(expiresAtMs);

  let memberCount = 0;
  let skipped = 0;
  const targets: FireTargetRecord[] = [];
  const tasks: Promise<MemberPlan>[] = [];
  ...
  const targetsMap: Record<string, FireTargetRecord> = {};
  targets.forEach((t, i) => {
  ...
  const all: PlannedWrite[] = [{ kind: "create", ref: fireRef, data: fireDoc }, ...writes];
  for (let i = 0; i < all.length; i += FANOUT_BATCH_SIZE) {
    const batch = db.batch();
    for (const w of all.slice(i, i + FANOUT_BATCH_SIZE)) {
      if (w.kind === "create") batch.create(w.ref, w.data);
  ...
  console.log(
    `applySyncPattern FANOUT: group=${args.groupId} fire=${fireId} ` +
