  ...
  async (request): Promise<BackfillResult> => {
    ...
    if (request.auth.token.admin !== true) {
      throw new HttpsError(
        "permission-denied",
        "backfillUserLocations requires the admin custom claim"
      );
    }
    ...
    try {
      ...
      logger.error("backfillUserLocations: scan failed", error);
      throw new HttpsError("internal", "Failed to scan /users");
    }

    logger.info(
      `backfillUserLocations: scanning ${snapshot.size} user docs`
    );
    ...
    for (const doc of snapshot.docs) {
      ...
      const address = (user.address as string | undefined)?.trim() ?? "";
      ...
      try {
        ...
        result.succeeded++;
        logger.info(
          `backfillUserLocations: ${doc.id} -> ${outcome.latitude},${outcome.longitude} (${outcome.timeZone ?? "no tz"})`
        );
      } catch (error) {
        const reason =
        ...
        logger.warn(`backfillUserLocations: ${doc.id} failed: ${reason}`);
      }
    }

    logger.info(
      `backfillUserLocations: done — processed=${result.processed} succeeded=${result.succeeded} failed=${result.failed} skipped=${result.skipped}`
    );
    return result;
  }
);
...
async function geocodeAddress(
  ...
  const geocodeBody = (await geocodeRes.json()) as {
    status: string;
  ...
  try {
    ...
    if (tzRes.ok) {
      const tzBody = (await tzRes.json()) as {
        status: string;
        timeZoneId?: string;
      };
    ...
    logger.warn("backfillUserLocations: timezone lookup failed", error);
  }
