...
| 1.3 | 5.1.1(v) in-app account deletion | **PASS** (was PARTIAL; corrected 2026-09-17) | The premise of the old finding is dead. The `user_service.dart` method it cited is now `@Deprecated` with an explicit "DO NOT use this for account deletion — Firestore does not cascade" warning ([user_service.dart:268-277](lib/services/user_service.dart#L268-L277)). The live path re-authenticates, purges via the `purgeUserAccount` callable, and only then deletes the Auth user ([security_settings_screen.dart](lib/features/site/security_settings_screen.dart)). DEPLOYMENT VERIFIED by `firebase functions:list` on 2026-09-17: `purgeUserAccount` and `scheduledDataCleanup` are both live in us-central1. |
| 1.4 | Privacy manifests | **PASS (app-level)** (was FAIL; corrected 2026-09-17) | The old claim ("no `PrivacyInfo.xcprivacy` anywhere in the repo") is verified false. `ios/Runner/PrivacyInfo.xcprivacy` exists, is git-tracked, and is wired into the Runner target's Resources build phase — `project.pbxproj` carries it as both a `PBXFileReference` and a `PBXBuildFile`, listed in `97C146EC1CF9000F007C117D /* Resources */`, so it is copied into the bundle rather than merely sitting in the folder. The plugin-level gap below stands unchanged: `path_provider_foundation`, `flutter_blue_plus` and `speech_to_text` still ship no manifest of their own. See F-9. |
...
| I-28 | Other callables enforce auth | **PASS** | | `claudeProxy` ([claudeProxy.js:46-47](functions/src/claudeProxy.js#L46-L47) — `if (!request.auth) throw unauthenticated`). **Corrected 2026-09-17:** this row used to cite `openaiProxy`, which no longer exists. It was deleted from source on this branch and confirmed absent from production by `firebase functions:list` (46 functions, no `openaiProxy`). `claudeProxy` is the AI path, and it enforces auth the same way, `processScheduleCommand` ([:656](functions/src/processScheduleCommand.ts#L656)), `sendSyncNotification` ([:34](functions/src/sendSyncNotification.ts#L34)), `redeemReferralCode`, `notifyDay2Team`, `notifyReferrerOfApproval`, `generateAlexaAuthCode`, and all three backfills (which additionally require an `admin: true` claim). `createCustomerAccount` is the sole gap. |
