...
| **Artifact location** | `C:\Flutter Projects\lumina-b97-artifacts\build\app\outputs\bundle\release\app-release.aab`, symbols in `C:\Flutter Projects\lumina-b97-artifacts\build\debug-info\android\`. A plain folder, **not** a git worktree, so `flutter clean` and worktree cleanup cannot reach it. Copied 2026-09-16 out of the `%TEMP%` build worktree; the copy re-verified at 68,611,733 bytes, sha256 `39ad7649e5e8ca87326913585bbee74efef515c744abc2a13f92a6ec61ac4d83`, and all three `.symbols` files sha256-match their originals. |
